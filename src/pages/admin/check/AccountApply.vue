 ...
 * @LastEditTime: 2021-06-26 17:21:35
 * @Description: 账户变更
-->
<template>
  <div>
    ...
    <el-table v-model="loading" :data="tableData" style="width: 100%">
      ...
      <!-- <el-table-column :prop="fmtData('applyTime')" label="申请时间">{{tableData.applyTime | fmtDate}} </el-table-column> -->
      ...
      <el-table-column prop="money" label="变更资金">
        <template
          v-slot="scope"
        >￥<span class="money_color">{{ scope.row.money }}</span> 元</template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async pageQueryData() {
      this.loading = true
      if (this.appTime && this.appTime.length > 0) {
        this.searchForm.beginTime = this.appTime[0]
        this.searchForm.endTime = this.appTime[1]
      }
      // 复制分页数据 页数
      const temp = {
        ...this.params,
      ...
      console.log(temp)
      const res = await pageQueryAccountApply(temp)
      this.loading = false
